...
class WebCrawlerToOpenAIClient:
    ...
    def scrape_and_rate_query(self, query, num_pages, start_n_days_ago, end_n_days_ago):
        summaries = self.crawler.crawl_query(query, num_pages, start_n_days_ago, end_n_days_ago)
        formatted_summaries = "\n\n".join(summaries)
        ...
        return sentiment_score, explanation, summary
...
date_intervals, sentiment_scores, explanations, summaries = [], [], [], []
...
for start_interval in range(start_n_days_ago, end_n_days_ago, -interval_size):
    ...
    sentiment_score, explanation, summary = web_crawler_to_open_ai_client.scrape_and_rate_query(query, num_pages_to_scrape, interval.start, interval.end)
...
for i, interval in enumerate(date_intervals):
    ...
    print("End: " + end_date)
    print("Sentiment Score: " + str(sentiment_scores[i]))
    print("Explanation: " + explanations[i])
    print("Summary: " + summaries[i])
    print("________")
    

# plt.plot(date_intervals, sentiment_scores)
